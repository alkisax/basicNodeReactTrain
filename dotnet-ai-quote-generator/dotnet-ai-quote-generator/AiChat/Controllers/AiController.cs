...
public class AiController
{
  ...
  public async Task<IResult> TestDeepSeekCall(HttpRequest request)
  {
    try
    {
      // appsettings
      ...
      int? userId = null;
      var userIdClaim = request.HttpContext.User.FindFirst("id")?.Value;
      if (int.TryParse(userIdClaim, out var parseUserId)) userId = parseUserId;

      var savedQuote = await _quoteDao.Create(quote, userId);
